  ...
  try {
    ...
    for (const [key, value] of formData.entries()) {
      if (value instanceof File) {
        ...
        const fileWithType = new File([typedBlob], value.name, {
          type: mimeType,
        });
    ...
    const response = await fetch(
      `${BACKEND_URL}/api/voice-to-text/transcribe-verbose`,
      {
        method: "POST",
        body: newFormData,
        headers: {
    ...
    console.error("Error proxying verbose transcription request:", error);
